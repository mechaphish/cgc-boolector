...
btor_const_aigvec (BtorAIGVecMgr * avmgr, BtorBitVector * bits)
{
  ...
  for (i = 0; i < len; i++)
    result->aigs[i] = !btor_get_bit_bv (bits, len-1-i)
      ? BTOR_AIG_FALSE : BTOR_AIG_TRUE;
  result->encoded = 1;
  return result;
}
...
BtorAIGVec *
btor_slice_aigvec (BtorAIGVecMgr * avmgr, BtorAIGVec * av,
...
btor_sll_n_bits_aigvec (BtorAIGVecMgr * avmgr,
                        BtorAIGVec * av, uint32_t n, BtorAIG * shift)
{
  BtorAIGMgr *amgr;
  BtorAIGVec *result;
...
static BtorAIGVec *
btor_srl_n_bits_aigvec (BtorAIGVecMgr * avmgr, BtorAIGVec * av, uint32_t n,
                        BtorAIG * shift)
{
  BtorAIGMgr *amgr;
  BtorAIGVec *result;
  ...
  for (i = 0; i < av->len; i++)
    {
      if (BTOR_IS_CONST_AIG (av->aigs[i]))
        ...
        {
          aig = av->aigs[i];
          assert (BTOR_REAL_ADDR_AIG (aig)->id <
...
BtorAIGVecMgr *
btor_clone_aigvec_mgr (BtorMemMgr * mm, BtorMsg * msg, BtorOpts * opts, BtorAIGVecMgr * avmgr)
{
  assert (mm);
  assert (msg);
  assert (opts);
