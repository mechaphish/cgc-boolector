...
btor_dump_aiger (Btor * btor, FILE * output, bool is_binary, bool merge_roots)
{
  ...
  BTOR_ABORT_DUMPAIG (btor->ops[BTOR_UF_NODE].cur > 0
                      || btor->ops[BTOR_LAMBDA_NODE].cur > 0,
  ...
  while (btor_has_next_node_hash_table_iterator (&it))
    {
      av = btor_exp_to_aigvec (btor, btor_next_node_hash_table_iterator (&it),
                               backannotation);
      assert (av->len == 1);
      if (merge_roots)
        {
  ...
  if (merge_roots)
    BTOR_PUSH_STACK (btor->mm, roots, merged);

  if (BTOR_EMPTY_STACK (roots))
    BTOR_PUSH_STACK (btor->mm, roots,
      btor->inconsistent ? BTOR_AIG_FALSE : BTOR_AIG_TRUE);

  btor_dump_seq_aiger (amgr,
    is_binary, output, BTOR_COUNT_STACK (roots), roots.start, 0, 0, 0,
    backannotation);
                     ...
                     BtorPtrHashTable * backannotation)
{
  ...
  for (i = 0; i < nregs; i++)
    {
      if (!binary)
        fprintf (file, "%u ", aiger_encode_aig (table, regs[i]));
  ...
  while (p)
    {
      ...
      if (left_id < right_id)
        BTOR_SWAP (int, left_id, right_id);
