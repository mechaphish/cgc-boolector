...
int
btor_precond_slice_exp_dbg (Btor * btor, BtorNode * exp,
...
int
btor_precond_regular_binary_bv_exp_dbg (Btor * btor, BtorNode * e0,
                                        BtorNode * e1)
{
  assert (btor);
  assert (e0);
...
btor_precond_read_exp_dbg (Btor * btor, BtorNode * e_array, BtorNode * e_index)
{
  ...
  assert (btor_get_index_array_sort (&btor->sorts_unique_table,
                                     e_array->sort_id)
...
int
btor_precond_write_exp_dbg (Btor * btor, BtorNode * e_array,
                            BtorNode * e_index, BtorNode * e_value)
{
  ...
  assert (btor_get_index_array_sort (&btor->sorts_unique_table,
                                     e_array->sort_id)
          == BTOR_REAL_ADDR_NODE (e_index)->sort_id);
  assert (btor_get_element_array_sort (&btor->sorts_unique_table,
                                       e_array->sort_id)
...
int
btor_precond_cond_exp_dbg (Btor * btor, BtorNode * e_cond,
...
is_sorted_bv_exp (Btor * btor, BtorNodeKind kind, BtorNode ** e)
{
  ...
  if (e[0] == e[1]) return 1;
  if (BTOR_INVERT_NODE (e[0]) == e[1]
      && BTOR_IS_INVERTED_NODE (e[1])) return 1;
...
{
  if (!is_sorted_bv_exp (btor, kind, e))
    BTOR_SWAP (BtorNode *, e[0], e[1]);
}
...
hash_bv_exp (Btor * btor, BtorNodeKind kind, int arity, BtorNode ** e)
{
  ...
  if (btor->options.sort_exp.val > 0
    ...
    assert (arity == 2),
    assert (BTOR_REAL_ADDR_NODE (e[0])->id <= BTOR_REAL_ADDR_NODE (e[1])->id);
#else
  (void) btor;
  (void) kind;
#endif
...
new_slice_exp_node (Btor * btor, BtorNode * e0, uint32_t upper, uint32_t lower)
{
  ...
  exp->upper = upper;
  exp->lower = lower;
  exp->sort_id = btor_bitvec_sort (&btor->sorts_unique_table,
                                   upper - lower + 1); 
  setup_node_and_add_to_id_table (btor, exp);
...
new_lambda_exp_node (Btor * btor, BtorNode * e_param, BtorNode * e_exp)
{
  ...
  domain = btor_tuple_sort (sorts, param_sorts.start,
                            BTOR_COUNT_STACK (param_sorts));
      ...
      if (BTOR_IS_ARGS_NODE (BTOR_REAL_ADDR_NODE (e[i])))
        {
          ...
          btor_init_tuple_sort_iterator (&it, &btor->sorts_unique_table,
                                         e[i]->sort_id);
          while (btor_has_next_tuple_sort_iterator (&it))
            BTOR_PUSH_STACK (btor->mm, sorts,
                             btor_next_tuple_sort_iterator (&it));
        }
      else
        BTOR_PUSH_STACK (btor->mm, sorts, BTOR_REAL_ADDR_NODE (e[i])->sort_id);
    }
  exp->sort_id = btor_tuple_sort (&btor->sorts_unique_table,
                     sorts.start, BTOR_COUNT_STACK (sorts));
  ...
  switch (kind)
    {
      ...
      case BTOR_CONCAT_NODE:
        sort = btor_bitvec_sort (sorts, btor_get_exp_width (btor, e[0])
                                        + btor_get_exp_width (btor, e[1]));
        break;

      case BTOR_FEQ_NODE:
      case BTOR_BEQ_NODE:
      case BTOR_ULT_NODE:
        sort = btor_bool_sort (sorts);
        break;
      ...
      while (btor_has_next_lambda_iterator (&it))
        {
          ...
          if (param->sort_id != subst_param->sort_id)
            goto NOT_EQUAL;
      ...
      if (!b)
      ...
      else if (!b->data.asPtr)
        {
          ...
          if (BTOR_IS_SLICE_NODE (real_cur))
            {
              result =
                find_slice_exp (btor, e[0], btor_slice_get_upper (real_cur),
...
BtorNode *
btor_array_exp (Btor * btor, uint32_t elem_width, uint32_t index_width,
                const char *symbol)
{
  assert (btor);
  assert (elem_width > 0);
...
btor_uf_exp (Btor * btor, BtorSortId sort, const char * symbol)
{
  ...
  assert (btor_is_bitvec_sort (sorts, btor_get_codomain_fun_sort (sorts, sort))
          || btor_is_bool_sort (sorts,
                 btor_get_codomain_fun_sort (sorts, sort)));
...
static BtorNode *
unary_exp_slice_exp (Btor * btor, BtorNode * exp,
                     uint32_t upper, uint32_t lower)
{
  assert (btor);
  assert (exp);
...
BtorNode *
btor_slice_exp_node (Btor * btor, BtorNode * exp,
                     uint32_t upper, uint32_t lower)
{
  exp = btor_simplify_exp (btor, exp);
      ...
      switch (kind)
        {
          ...
          case BTOR_ARGS_NODE:
            *lookup = new_args_exp_node (btor, arity, simp_e);
            break;
          default:
            *lookup = new_node (btor, kind, arity, simp_e);
        }
      assert (btor->nodes_unique_table.num_elements < INT_MAX);
      btor->nodes_unique_table.num_elements++;
      (*lookup)->unique = 1;
                    ...
                    BtorNode * e_else)
{
  ...
  if (BTOR_IS_FUN_NODE (BTOR_REAL_ADDR_NODE (e[1]))
      && (e[1]->parameterized || e[2]->parameterized))
    {
      ...
      for (i = 0; i < arity; i++)
        {
          width = btor_get_width_bitvec_sort (sorts,
                      sort->fun.domain->tuple.elements[i]->id);
          BTOR_PUSH_STACK (btor->mm,
                           params,
                           btor_param_exp (btor, width, 0));
        }
...
btor_smulo_exp (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  if (width == 2)
  ...
  else
    {
      ...
      for (i = 1; i < width - 2; i++)
        {
          slice = btor_slice_exp (
              btor, xor_sign_e2, width - 2 - i, width - 2 - i);
          temps_e2[i] = btor_or_exp (btor, temps_e2[i - 1], slice);
          btor_release_exp (btor, slice);
        }
...
btor_get_fun_exp_width (Btor * btor, BtorNode * exp)
{
  ...
  return btor_get_width_bitvec_sort (sorts,
           btor_get_codomain_fun_sort (sorts, exp->sort_id));
}
...
btor_get_index_exp_width (Btor * btor, BtorNode * e_array)
{
  ...
  return btor_get_width_bitvec_sort (sorts,
             btor_get_index_array_sort (sorts, e_array->sort_id));
}
