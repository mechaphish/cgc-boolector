...
btor_compute_scores (Btor * btor)
{
  ...
  if (btor->options.just_heuristic.val == BTOR_JUST_HEUR_LEFT)
    return;
          ...
          for (i = 0; i < cur->arity; i++)
            {
              ...
              if (!cur->parameterized 
                ...
                {
                  btor_insert_in_ptr_hash_table (
                      slv->score, btor_copy_exp (btor, e));
          ...
          if (BTOR_IS_APPLY_NODE (cur)
              || BTOR_IS_FEQ_NODE (cur))
            {
              ...
              if (!btor_find_in_ptr_hash_table (slv->score, cur))
                {
                  btor_insert_in_ptr_hash_table (
                      slv->score, btor_copy_exp (btor, cur));
                  /* push onto working stack */
