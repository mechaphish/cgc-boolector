...
is_const_one_exp (Btor * btor, BtorNode * exp)
{
  ...
  if (BTOR_IS_INVERTED_NODE (exp))
    bits = btor_not_bv (btor->mm, bits);
  result = btor_is_special_const_bv (bits) == BTOR_SPECIAL_CONST_BV_ONE;
  if (BTOR_IS_INVERTED_NODE (exp))
...
is_const_zero_exp (Btor * btor, BtorNode * exp)
{
  ...
  if (BTOR_IS_INVERTED_NODE (exp))
    result = btor_is_ones_bv (btor_const_get_bits (exp));
  else
    result = btor_is_zero_bv (btor_const_get_bits (exp));
...
is_const_zero_or_ones_exp (Btor * btor, BtorNode * exp)
{
  ...
  result = btor_is_zero_bv (bits) || btor_is_ones_bv (bits);
...
is_odd_const_exp (BtorNode * exp)
{
  ...
  bits = btor_const_get_bits (exp);
  return btor_get_bit_bv (bits, 0) == 1;
}
...
slice_simplifiable (BtorNode * exp)
{
  ...
  return BTOR_IS_BV_VAR_NODE (exp)
         || BTOR_IS_BV_CONST_NODE (exp)
...
is_true_cond (BtorNode * cond)
{
  ...
  if (BTOR_IS_INVERTED_NODE (cond)
      && !btor_get_bit_bv (btor_const_get_bits (cond), 0))
    return true;
                        ...
                        BtorNode * e1)
{
  ...
  switch (kind)
    {
      case BTOR_AND_NODE:
        bresult = btor_and_bv (mm, b0, b1);
        break;
      case BTOR_BEQ_NODE:
        bresult = btor_eq_bv (mm, b0, b1);
        break;
      case BTOR_ADD_NODE:
        bresult = btor_add_bv (mm, b0, b1);
        break;
      case BTOR_MUL_NODE:
        bresult = btor_mul_bv (mm, b0, b1);
        break;
      case BTOR_ULT_NODE:
        bresult = btor_ult_bv (mm, b0, b1);
        break;
      case BTOR_UDIV_NODE:
        bresult = btor_udiv_bv (mm, b0, b1);
        break;
      case BTOR_UREM_NODE:
        bresult = btor_urem_bv (mm, b0, b1);
        break;
      case BTOR_SLL_NODE:
        bresult = btor_sll_bv (mm, b0, b1);
        break;
      case BTOR_SRL_NODE:
        bresult = btor_srl_bv (mm, b0, b1);
        break;
      default:
                                    ...
                                    BtorNode * e0, BtorNode * e1)
{
  ...
  if (invert_b0) b0 = btor_not_bv (mm, b0);
  sc = btor_is_special_const_bv (b0);
  if (invert_b0) btor_free_bv (mm, b0);
      ...
      default:
        ...
        if (kind == BTOR_BEQ_NODE && real_e1->kind == BTOR_AND_NODE &&
            btor->rec_rw_calls < BTOR_REC_RW_BOUND)
          {
            ...
            if (BTOR_IS_INVERTED_NODE (e0))
              bv = btor_not_bv (btor->mm, btor_const_get_bits (real_e0));
            else
              bv = btor_copy_bv (btor->mm, btor_const_get_bits (real_e0));
            ...
            if (BTOR_IS_INVERTED_NODE (e1))
              {
                while (pos < width_e0)
                  {
                    bvstr = btor_bv_to_char_bv (btor->mm, bv);
                    tmpstr[0] = bvstr[pos];
                    ...
                    if (tmpstr[0] == '0')
                      {
                        ...
                        BTOR_PUSH_STACK (btor->mm, stack,
                          rewrite_eq_exp (btor, tmp1, tmp3));
                        BTOR_PUSH_STACK (btor->mm, stack,
                          rewrite_eq_exp (btor, tmp2, tmp3));
                    ...
                    else
                      {
                        ...
                        BTOR_PUSH_STACK (btor->mm,
                          stack, rewrite_eq_exp (btor, tmp3, tmp4));
                        btor_release_exp (btor, tmp3);
                        btor_release_exp (btor, tmp4);
                      }
                ...
                while (pos < width_e0)
                  {
                    ...
                    len = (int) strspn (bvstr + pos, tmpstr);
                    btor_freestr (btor->mm, bvstr);
                    tmp1 = rewrite_slice_exp (btor, e1->e[0],
                                              width_e0 - 1 - pos,
                                              width_e0 - pos - len);
                    tmp2 = rewrite_slice_exp (btor, e1->e[1],
                                              width_e0 - 1 - pos,
                                              width_e0 - pos - len);
                    if (tmpstr[0] == '1')
                      {
                        ...
                        BTOR_PUSH_STACK (btor->mm,
                          stack, rewrite_eq_exp (btor, tmp1, tmp3));
                        BTOR_PUSH_STACK (btor->mm,
                          stack, rewrite_eq_exp (btor, tmp2, tmp3));
                        btor_release_exp (btor, tmp3);
                      }
                    else
                      {
 ...
 */
static inline int 
applies_special_const_rhs_binary_exp (Btor * btor, BtorNodeKind kind,
                                      BtorNode * e0, BtorNode * e1)
{
  (void) btor;
  (void) kind;
...
static inline BtorNode *
apply_special_const_rhs_binary_exp (Btor * btor, BtorNodeKind kind,
                                    BtorNode * e0, BtorNode * e1)
{
  assert (applies_special_const_rhs_binary_exp (btor, kind, e0, e1));

  char tmpstr[2] = { '\0', '\0'}, *bvstr;
  int pos, len, width_e0, width_e1;
  bool invert_b1;
  BtorBitVector *b1, *bv;
  BtorMemMgr *mm;
  BtorSpecialConstBitVector sc;
  ...
  if (invert_b1) b1 = btor_not_bv (mm, b1);
  sc = btor_is_special_const_bv (b1);
  if (invert_b1) btor_free_bv (mm, b1);
      ...
      switch (kind)
        {
          ...
          case BTOR_ULT_NODE:        /* x < 0 */
            ...
            break;
          case BTOR_UDIV_NODE:
            result = btor_ones_exp (btor, width_e0);
            break;
          default:
            break;
        }
    ...
    default:
      ...
      if (kind == BTOR_BEQ_NODE && real_e0->kind == BTOR_AND_NODE &&
          btor->rec_rw_calls < BTOR_REC_RW_BOUND)
        {
          ...
          BTOR_INIT_STACK (stack);
          if (BTOR_IS_INVERTED_NODE (e1))
            bv = btor_not_bv (btor->mm, btor_const_get_bits (real_e1));
          else
            bv = btor_copy_bv (btor->mm, btor_const_get_bits (real_e1));
              ...
              while (pos < width_e1)
                {
                  ...
                  len = (int) strspn (bvstr + pos, tmpstr);
                  btor_freestr (btor->mm, bvstr);
                  tmp1 = rewrite_slice_exp (btor, e0->e[0],
                                            width_e1 - 1 - pos,
                                            width_e1 - pos - len);
                  tmp2 = rewrite_slice_exp (btor, e0->e[1],
                                            width_e1 - 1 - pos,
                                            width_e1 - pos - len);
                  if (tmpstr[0] == '1')
                    {
  ...
  if (BTOR_IS_INVERTED_NODE (term))
    {
      ...
      if (!rewrite_linear_term_bounded (
          btor, BTOR_INVERT_NODE (term), &factor, lhs_ptr, rhs_ptr, bound_ptr))
        return 0;

      *rhs_ptr = BTOR_INVERT_NODE (*rhs_ptr);
      *factor_ptr = btor_neg_bv (btor->mm, factor);
      btor_free_bv (btor->mm, factor);
    }
 ...
 */
static inline int
applies_const_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                     uint32_t lower)
{
  (void) btor;
  (void) upper;
...
apply_const_slice (Btor * btor, BtorNode * exp, uint32_t upper, uint32_t lower)
{
  ...
  bits = btor_slice_bv (btor->mm, btor_const_get_bits (exp), upper, lower);
 ...
 */
static inline int
applies_slice_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                     uint32_t lower)
{
  (void) upper;
  (void) lower;
...
apply_slice_slice (Btor * btor, BtorNode * exp, uint32_t upper, uint32_t lower)
{
  ...
  real_exp = BTOR_REAL_ADDR_NODE (exp);
  BTOR_INC_REC_RW_CALL (btor);
  result = rewrite_slice_exp (btor, BTOR_COND_INVERT_NODE (exp, real_exp->e[0]),
 ...
 */
static inline int
applies_concat_lower_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                            uint32_t lower)
{
  (void) btor;
...
static inline BtorNode *
apply_concat_lower_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                          uint32_t lower)
{
 ...
 */
static inline int
applies_concat_upper_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                            uint32_t lower)
{
  return BTOR_IS_CONCAT_NODE (BTOR_REAL_ADDR_NODE (exp))
...
static inline BtorNode *
apply_concat_upper_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                          uint32_t lower)
{
 ...
 */
static inline int
applies_concat_rec_upper_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                                uint32_t lower)
{
  (void) upper;
...
static inline BtorNode *
apply_concat_rec_upper_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                              uint32_t lower)
{
 ...
 */
static inline int
applies_concat_rec_lower_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                                uint32_t lower)
{
  (void) lower;
  return btor->options.rewrite_level.val >= 3
...
static inline BtorNode *
apply_concat_rec_lower_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                              uint32_t lower)
{
 ...
 */
static inline int
applies_concat_rec_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                          uint32_t lower)
{
  return BTOR_IS_CONCAT_NODE (BTOR_REAL_ADDR_NODE (exp))
         && btor->options.rewrite_level.val >= 3
...
static inline BtorNode *
apply_concat_rec_slice (Btor * btor, BtorNode * exp, uint32_t upper,
                        uint32_t lower)
{
  ...
  len = btor_get_exp_width (btor, real_exp->e[1]);
  BTOR_INC_REC_RW_CALL (btor);
  tmp = rewrite_slice_exp (btor,
          BTOR_COND_INVERT_NODE (exp, real_exp->e[0]),
          upper - len, 0);
  result = rewrite_concat_exp (btor, tmp,
...
static inline int
applies_zero_lower_slice (Btor * btor, BtorNode * exp, uint32_t upper, uint32_t lower)
{
  (void) upper;
  ...
  if (BTOR_IS_INVERTED_NODE (e1))
    {
      ...
      result = rewrite_and_exp (btor,
                                     BTOR_INVERT_NODE (real_e1->e[0]), tmp);
    }
  else
    {
...
apply_subst1_and (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  BTOR_INC_REC_RW_CALL (btor);
  result = rewrite_and_exp (btor, e0,
                            BTOR_INVERT_NODE (BTOR_REAL_ADDR_NODE (e1)->e[1]));
  BTOR_DEC_REC_RW_CALL (btor);
  return result;
}
...
apply_subst2_and (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  BTOR_INC_REC_RW_CALL (btor);
  result = rewrite_and_exp (btor, e0,
                            BTOR_INVERT_NODE (BTOR_REAL_ADDR_NODE (e1)->e[0]));
  BTOR_DEC_REC_RW_CALL (btor);
  return result;
}
...
apply_resol1_and (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  (void) e1;
  return BTOR_INVERT_NODE (btor_copy_exp (btor,
...
applies_subsum2_and (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  BtorNode *real_e0;
  real_e0 = BTOR_REAL_ADDR_NODE (e0);
  return BTOR_IS_AND_NODE (real_e0)
         && BTOR_IS_INVERTED_NODE (e0)
...
apply_subst3_and (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  BTOR_INC_REC_RW_CALL (btor);
  result = rewrite_and_exp (btor,
               BTOR_INVERT_NODE (BTOR_REAL_ADDR_NODE (e0)->e[0]), e1);
  BTOR_DEC_REC_RW_CALL (btor);
  return result;
}
...
applies_ult_and (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  real_e1 = BTOR_REAL_ADDR_NODE (e1);
  return btor->rec_rw_calls < BTOR_REC_RW_BOUND
         && BTOR_IS_ULT_NODE (real_e0)
         && BTOR_IS_ULT_NODE (real_e1)
         && BTOR_IS_INVERTED_NODE (e0)
         && BTOR_IS_INVERTED_NODE (e1)
         && real_e0->e[0] == real_e1->e[1]
         && real_e0->e[1] == real_e1->e[0];
...
{
  (void) btor;
  return BTOR_REAL_ADDR_NODE (e0) == BTOR_REAL_ADDR_NODE (e1)
         && e0 != e1;
}
...
apply_const_concat (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  BTOR_INC_REC_RW_CALL (btor);
  tmp = rewrite_concat_exp (btor,
            BTOR_COND_INVERT_NODE (e0, real_e0->e[1]), e1);
  result = rewrite_concat_exp (btor,
             BTOR_COND_INVERT_NODE (e0, real_e0->e[0]), tmp);
  btor_release_exp (btor, tmp);
  BTOR_DEC_REC_RW_CALL (btor);
  return result;
}
...
applies_slice_concat (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  return btor->options.rewrite_level.val > 0
         ...
         && BTOR_IS_SLICE_NODE (real_e0)
         && BTOR_IS_SLICE_NODE (real_e1)
         && real_e0->e[0] == real_e1->e[0]
...
apply_slice_concat (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  real_e0 = BTOR_REAL_ADDR_NODE (e0);
  BTOR_INC_REC_RW_CALL (btor);
  result = rewrite_slice_exp (btor, real_e0->e[0],
...
apply_and_lhs_concat (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  real_e0 = BTOR_REAL_ADDR_NODE (e0);
  BTOR_INC_REC_RW_CALL (btor);
  left = rewrite_concat_exp (btor, real_e0->e[0],
                             BTOR_COND_INVERT_NODE (e0, e1));
  right = rewrite_concat_exp (btor, real_e0->e[1],
                              BTOR_COND_INVERT_NODE (e0, e1));
  result = btor_and_exp (btor, left, right);
  result = BTOR_COND_INVERT_NODE (e0, result);
  btor_release_exp (btor, right);
...
apply_and_rhs_concat (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  real_e1 = BTOR_REAL_ADDR_NODE (e1);
  BTOR_INC_REC_RW_CALL (btor);
  left = rewrite_concat_exp (btor,
                             BTOR_COND_INVERT_NODE (e1, e0), real_e1->e[0]);
  right = rewrite_concat_exp (btor,
                              BTOR_COND_INVERT_NODE (e1, e0), real_e1->e[1]);
  result = btor_and_exp (btor, left, right);
...
apply_const_sll (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  bits = btor_const_get_bits (real_e1);
  if (BTOR_IS_INVERTED_NODE (e1)) bits = btor_not_bv (btor->mm, bits);
  shiftlen = (unsigned) btor_bv_to_uint64_bv (bits);
  if (BTOR_IS_INVERTED_NODE (e1)) btor_free_bv (btor->mm, bits);
  assert (shiftlen > 0);
  assert (shiftlen < btor_get_exp_width (btor, real_e0));
  BTOR_INC_REC_RW_CALL (btor);
  pad = btor_zero_exp (btor, shiftlen);
  slice = rewrite_slice_exp (btor, e0,
            btor_get_exp_width (btor, real_e0) - shiftlen - 1, 0);
...
apply_param_lambda_apply (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  result = btor_copy_exp (btor,
               btor_param_cur_assignment (BTOR_REAL_ADDR_NODE (body)));
          ...
          if (BTOR_IS_BV_CONST_NODE (BTOR_REAL_ADDR_NODE (beta_cond)))
            {
              ...
              /* propagate down to next function */
                ...
                {
                  args =
                    btor_beta_reduce_bounded (btor, real_cur_branch->e[1], 1);
                      ...
                      if (BTOR_IS_APPLY_NODE (BTOR_REAL_ADDR_NODE (result)))
                        {
                          ...
                          args = btor_copy_exp (btor,
                                     BTOR_REAL_ADDR_NODE (result)->e[1]);
 ...
 */
static inline int
applies_equal_branches_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                             BtorNode * e2)
{
  (void) btor;
  (void) e0;
...
static inline BtorNode * 
apply_equal_branches_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                           BtorNode * e2)
{
...
apply_const_cond (Btor * btor, BtorNode * e0, BtorNode * e1, BtorNode * e2)
{
  ...
  if (btor_get_bit_bv (btor_const_get_bits (e0), 0))
    return btor_copy_exp (btor, e1);
 ...
 */
static inline int
applies_cond_if_dom_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                          BtorNode * e2)
{
  (void) e2;
  BtorNode *real_e1;
  real_e1 = BTOR_REAL_ADDR_NODE (e1);
  return btor->rec_rw_calls < BTOR_REC_RW_BOUND
         && BTOR_IS_COND_NODE (real_e1)
         && real_e1->e[0] == e0;
}

static inline BtorNode * 
apply_cond_if_dom_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                        BtorNode * e2)
{
  ...
  BTOR_INC_REC_RW_CALL (btor);
  result = rewrite_cond_exp (btor, e0,
               BTOR_COND_INVERT_NODE (e1, BTOR_REAL_ADDR_NODE (e1)->e[1]), e2);
  BTOR_DEC_REC_RW_CALL (btor);
  return result;
}
 ...
 */
static inline int
applies_cond_if_merge_if_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                               BtorNode * e2)
{
  (void) e0;
  BtorNode *real_e1;
...
static inline BtorNode * 
apply_cond_if_merge_if_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                             BtorNode * e2)
{
 ...
 */
static inline int
applies_cond_if_merge_else_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                                 BtorNode * e2)
{
  (void) e0;
  BtorNode *real_e1;
...
static inline BtorNode * 
apply_cond_if_merge_else_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                               BtorNode * e2)
{
 ...
 */
static inline int
applies_cond_else_dom_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                          BtorNode * e2)
{
  (void) e1;
  BtorNode *real_e2;
  ...
  return btor->rec_rw_calls < BTOR_REC_RW_BOUND
         && BTOR_IS_COND_NODE (real_e2)
         && real_e2->e[0] == e0;
}

static inline BtorNode * 
apply_cond_else_dom_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                          BtorNode * e2)
{
  ...
  BTOR_INC_REC_RW_CALL (btor);
  result = rewrite_cond_exp (btor, e0, e1,
               BTOR_COND_INVERT_NODE (e2, BTOR_REAL_ADDR_NODE (e2)->e[2]));
  BTOR_DEC_REC_RW_CALL (btor);
  return result;
}
 ...
 */
static inline int
applies_cond_else_merge_if_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                                 BtorNode * e2)
{
  (void) e0;
  BtorNode *real_e2;
...
static inline BtorNode * 
apply_cond_else_merge_if_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                               BtorNode * e2)
{
 ...
 */
static inline int
applies_cond_else_merge_else_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                                   BtorNode * e2)
{
  (void) e0;
  BtorNode *real_e2;
...
static inline BtorNode * 
apply_cond_else_merge_else_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                                 BtorNode * e2)
{
...
applies_add_else_cond (Btor * btor, BtorNode * e0, BtorNode * e1, BtorNode * e2)
{
  (void) e0;
  return btor->rec_rw_calls < BTOR_REC_RW_BOUND
         && !BTOR_IS_INVERTED_NODE (e2)
         && BTOR_IS_ADD_NODE (e2)
         && ((e2->e[0] == e1
              && is_const_one_exp (btor, e2->e[1]))
             || (e2->e[1] == e1
                 && is_const_one_exp (btor, e2->e[0])));
}
...
apply_add_else_cond (Btor * btor, BtorNode * e0, BtorNode * e1, BtorNode * e2)
{
  ...
  BTOR_INC_REC_RW_CALL (btor);
  tmp = btor_uext_exp (btor, BTOR_INVERT_NODE (e0),
                       btor_get_exp_width (btor, e1) - 1);
 ...
 */
static inline int
applies_comm_op_1_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                        BtorNode * e2)
{
  (void) e0;
  return btor->options.rewrite_level.val > 2
 ...
 */
static inline int
applies_comm_op_2_cond (Btor * btor, BtorNode * e0, BtorNode * e1,
                        BtorNode * e2)
{
  (void) e0;
  return btor->options.rewrite_level.val > 2
...
static void
normalize_bin_comm_ass_exp (Btor * btor, BtorNode * e0, BtorNode * e1,
...
rebuild_top_add (Btor * btor, BtorNode * e, BtorNode * c, BtorNode * r)
{
  ...
  if (BTOR_IS_INVERTED_NODE (e))
  ...
  else 
    {
      ...
      res = rewrite_slice_exp (btor, tmp, btor_slice_get_upper (e),
                               btor_slice_get_lower (e));
      btor_release_exp (btor, tmp);
    }
  return res;
}
...
normalize_eq (Btor * btor, BtorNode ** left, BtorNode ** right)
{
  ...
  // TODO (ma): what does this do?
  if (btor->options.rewrite_level.val > 2
      && (c0 = find_top_add (btor, e0))
      && (c1 = find_top_add (btor, e1))
          ...
          if (BTOR_IS_CONCAT_NODE (real_cur))
            {
              BTOR_PUSH_STACK (mm, stack,
                               BTOR_COND_INVERT_NODE (cur, real_cur->e[1]));
              BTOR_PUSH_STACK (mm, stack,
                               BTOR_COND_INVERT_NODE (cur, real_cur->e[0]));
            }
          else
...
static inline void
normalize_cond (Btor * btor, BtorNode ** cond, BtorNode ** left,
                BtorNode ** right)
{
  BtorNode *e0, *e1, *e2;
...
rewrite_eq_exp (Btor * btor, BtorNode * e0, BtorNode * e1)
{
  ...
  kind = BTOR_IS_FUN_NODE (BTOR_REAL_ADDR_NODE (e0))
            ? BTOR_FEQ_NODE : BTOR_BEQ_NODE;
...
// FIXME: this rule may yield lambdas with differents sorts (in case of curried
//        lambdas)
...
BtorNode *
btor_rewrite_slice_exp (Btor * btor, BtorNode * exp, uint32_t upper,
                        uint32_t lower)
{
  assert (btor);
  assert (btor->options.rewrite_level.val > 0);
  ...
  switch (kind)
    {
      case BTOR_FEQ_NODE:
      case BTOR_BEQ_NODE:
        return rewrite_eq_exp (btor, e0, e1);

      case BTOR_ULT_NODE:
        return rewrite_ult_exp (btor, e0, e1);

      case BTOR_AND_NODE:
        return rewrite_and_exp (btor, e0, e1);

      case BTOR_ADD_NODE:
        return rewrite_add_exp (btor, e0, e1);

      case BTOR_MUL_NODE:
        return rewrite_mul_exp (btor, e0, e1);

      case BTOR_UDIV_NODE:
        return rewrite_udiv_exp (btor, e0, e1);

      case BTOR_UREM_NODE:
        return rewrite_urem_exp (btor, e0, e1);

      case BTOR_CONCAT_NODE:
        return rewrite_concat_exp (btor, e0, e1);

      case BTOR_SLL_NODE:
        return rewrite_sll_exp (btor, e0, e1);

      case BTOR_SRL_NODE:
        return rewrite_srl_exp (btor, e0, e1);

      case BTOR_APPLY_NODE:
        return rewrite_apply_exp (btor, e0, e1);
...
BtorNode *
btor_rewrite_ternary_exp (Btor * btor,
                          BtorNodeKind kind,
                          BtorNode * e0,
                          BtorNode * e1,
                          BtorNode * e2)
