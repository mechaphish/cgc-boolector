...
#define BTORMBT_LOG_STATUS(l, prefix) \
  BTORMBT_LOG (l, prefix " (%d): bool %ld, bv %ld, array %ld, fun %ld, uf %ld",\
               g_btormbt->ops, \
...
btormbt_new_btormbt (void)
{
  ...
  for (opt = boolector_first_opt (tmpbtor); opt;
       opt = boolector_next_opt (tmpbtor, opt))
    {
      ...
      if (!strcmp (opt, "incremental"))
        btoropt->min = btoropt->max;
...
btormbt_print_stats (BtorMBT * mbt)
{
  ...
  btormbt_msg ("%d rounds = %0.2f rounds per second",
               mbt->rounds, average (mbt->rounds, t));
  btormbt_msg ("%d bugs = %0.2f bugs per second",
               mbt->bugs, average (mbt->bugs, t));
...
catch_alarm (int sig)
{
  ...
  if (g_set_alarm > 0)
    {
      btormbt_msg ("ALARM TRIGGERED: time limit %d seconds reached",
                   g_set_alarm);
      btormbt_print_stats (g_btormbt);
    }
  reset_alarm ();
 ...
 */
static void
init_pd_inputs (BtorMBT * mbt, float ratio_var, float ratio_const,
                float ratio_arr)
{
  float sum;
 ...
 */
static void
init_pd_add (BtorMBT * mbt, float ratio_fun, float ratio_uf,
             float ratio_arr_op, float ratio_bv_op, float ratio_input)
{
  float sum;
  ...
  mbt->p_bitvec_uf = (ratio_uf * NORM_VAL) / sum;  // TODO (ma): UF should be input
  ...
  for (i = 0; i < num_nodes; i++)
    {
      ...
      node = boolector_copy (mbt->btor,
                             BTOR_PEEK_STACK (mbt->bo->exps, pos)->exp);
  ...
  switch (op)
    {
      ...
      default:
        assert (op == REDAND);
        node = boolector_redand (mbt->btor, e);
    }
  assert (node);
  btormbt_push_node (mbt, node);
  g_btormbtstats->num_ops[op]++;
  ...
  switch (op)
    {
      ...
      default:
        assert (op == IFF);
        node = boolector_iff (mbt->btor, e0, e1);
    }
  assert (node);
  btormbt_push_node (mbt, node);
  g_btormbtstats->num_ops[op]++;
  ...
  switch (op)
    {
      case WRITE:
        rand = pick (rng, 1, 2);
        e0 = select_exp (mbt, rng, BTORMBT_ARR_T,
      ...
      default:
        ...
        e1 = select_arr_exp (mbt, rng, e0,
                             boolector_get_width (mbt->btor, e0),
                             boolector_get_index_width (mbt->btor, e0), -1);
  ...
  if (rand < mbt->p_apply_fun
          ...
          || (mbt->paramfun && !BTOR_EMPTY_STACK (mbt->paramfun->exps))))
    {
      ...
      for (i = 0; i < (int) sort->fun.domain->tuple.num_elements; i++)
        {
          BTOR_PUSH_STACK (mbt->mm, param_widths,
              btor_get_width_bitvec_sort (
                  &mbt->btor->sorts_unique_table,
  ...
  else  /* generate new function */
    {
      ...
      for (i = 0; i < pick (&rng, MIN_NPARAMS, MAX_NPARAMS); i++)
        {
          ...
          width =
            pick (&rng, mbt->g_min_bw > 2 ? mbt->g_min_bw : 1, mbt->g_max_bw);
          tmp = boolector_param (mbt->btor, width, 0);
          BTOR_PUSH_STACK (mbt->mm, params, tmp);
          BTOR_PUSH_STACK (mbt->mm, param_widths,
                           boolector_get_width (mbt->btor, tmp));
      ...
      if (BTOR_EMPTY_STACK (mbt->paramarr->exps))
        {
          ...
          e0 = select_arr_exp (mbt, &rng, 0,
                               boolector_get_width (mbt->btor, e2),
                               boolector_get_width (mbt->btor, e1), -1);
          assert (btormbt_is_parameterized (mbt, e1)
      ...
      fun = boolector_fun (mbt->btor, params.start,
                           BTOR_COUNT_STACK (params), tmp);
...
//      btormbt_push_exp_stack (mbt->mm, mbt->uf, uf);
  ...
  btor_init_tuple_sort_iterator (&it, sorts,
      btor_get_domain_fun_sort (sorts, ((BtorNode *) uf)->sort_id));
  ...
  /* create apply on UF */
  apply = boolector_apply (mbt->btor, stack.start,
                           BTOR_COUNT_STACK (stack), uf);
...
btormbt_state_new (BtorMBT * mbt, unsigned r)
{
  ...
  mbt->max_inputs = 
    pick (&rng, mbt->g_min_inputs, mbt->g_max_inputs);
  /* number of initial operations */
  mbt->max_ops = 
    pick (&rng, mbt->g_min_ops_init, mbt->g_max_ops_init);
  ...
  init_pd_ops (mbt, 
      ...
      pick (&rng, mbt->g_min_release_ops_init,
            mbt->g_max_release_ops_init));

  /* no additional inputs at init */
  init_pd_add (mbt,
               pick (&rng, 
                      mbt->g_min_add_funs_init, 
                      mbt->g_max_add_funs_init),
               pick (&rng, 
                      mbt->g_min_add_uf_init, 
                      mbt->g_max_add_uf_init),
               pick (&rng, 
                      mbt->g_min_add_arrayops_init, 
                      mbt->g_max_add_arrayops_init),
               pick (&rng, 
                      mbt->g_min_add_bitvecops_init, 
                      mbt->g_max_add_bitvecops_init),
               pick (&rng, 
                      mbt->g_min_add_inputs_init, 
                      mbt->g_max_add_inputs_init));

  BTORMBT_LOG (1, 
      "new: pick %d ops (add:rel=%0.1f%%:%0.1f%%), %d inputs",
       mbt->max_ops, mbt->p_add / 10, mbt->p_release / 10, 
       mbt->max_inputs);
  ...
  for (i = 0; i < BTOR_COUNT_STACK (mbt->btor_opts); i++)
    {
      ...
      BTORMBT_LOG (1, "opt: set boolector option '%s' to '%d'",
                   btoropt->name, btoropt->val);
...
btormbt_state_init (BtorMBT * mbt, unsigned r)
{
  ...
  if (mbt->max_ops < mbt->g_max_ops_lower)
    mbt->max_ass = 
      BTORMBT_MIN (mbt->max_ops, 
                   pick (&rng, 
                         mbt->g_min_asserts_lower, 
                         mbt->g_max_asserts_lower));
  else
    mbt->max_ass = pick (&rng,   
                              mbt->g_min_asserts_upper, 
                              mbt->g_max_asserts_upper);
...
btormbt_state_main (BtorMBT * mbt, unsigned r)
{
  ...
  BTORMBT_LOG (1, "main: asserts %d, assumes %d",
               mbt->tot_asserts, mbt->assumes);

  if (mbt->dump)
    return btormbt_state_dump;
...
btormbt_state_add (BtorMBT * mbt, unsigned r)
{
  ...
  if (rand < mbt->p_bitvec_op)
    next = btormbt_state_bv_op;
           ...
           && rand < mbt->p_bitvec_op + mbt->p_array_op)
    ...
    next = btormbt_state_bv_fun;
           ...
           && rand < mbt->p_bitvec_op + mbt->p_array_op +
              mbt->p_bitvec_fun + mbt->p_bitvec_uf)
...
btormbt_state_bv_op (BtorMBT * mbt, unsigned r)
{
  ...
  if (is_unary_op (op))
    ...
    {
      e0 = select_exp (mbt, &rng, ((op >= IMPLIES && op <= IFF)
                                   ? BTORMBT_BO_T : BTORMBT_BB_T), 0);
      e1 = select_exp (mbt, &rng, ((op >= IMPLIES && op <= IFF)
                                   ? BTORMBT_BO_T : BTORMBT_BB_T), 0);
  ...
  if (rand < mbt->p_rw)
    {
      ...
      if (op == WRITE)
        e2 = select_exp (mbt, &rng, BTORMBT_BV_T, 0);
      btormbt_array_op (mbt, &rng, op, e0, e1, e2);
    }
  else
    {
      ...
      e1 = select_arr_exp (mbt, &rng, e0, e0w, e0iw,
                           op == EQ || op == NE ? -1 : 0);
      if (op == COND)
        e2 = select_exp (mbt, &rng, BTORMBT_BO_T, 0);
      btormbt_array_op (mbt, &rng, op, e0, e1, e2);
    }
...
btormbt_state_input (BtorMBT * mbt, unsigned r)
{
  ...
  // TODO (ma): UFs?
  rand = pick (&rng, 0, NORM_VAL - 1);
  if (mbt->create_arrays && rand < mbt->p_array)
    btormbt_array (mbt, &rng);
  if (rand < mbt->p_var)
...
btormbt_state_sat (BtorMBT * mbt, unsigned r)
{
  ...
  return mbt->mgen && res == BOOLECTOR_SAT
         ? btormbt_state_model_gen : btormbt_state_model_inc;
}
...
btormbt_state_model_gen (BtorMBT * mbt, unsigned r)
{
  ...
  for (i = 0; i < BTOR_COUNT_STACK (mbt->arr->exps); i++)
    {
      boolector_array_assignment (mbt->btor, mbt->arr->exps.start[i]->exp, 
          &indices, &values, &size);
      if (size > 0)
  ...
  for (i = 0; i < BTOR_COUNT_STACK (mbt->uf->exps); i++)
    {
      boolector_uf_assignment (mbt->btor, mbt->uf->exps.start[i]->exp, 
          &indices, &values, &size);
      if (size > 0)
  ...
  if (mbt->inc && rand < mbt->p_inc)
    {
      ...
      mbt->max_ops = 
        pick (&rng, mbt->g_min_ops_inc, mbt->g_max_ops_inc);
      ...
      init_pd_ops (mbt, 
          ...
          pick (&rng, mbt->g_min_release_ops_inc,
                mbt->g_max_release_ops_inc));
      init_pd_add (mbt,
                   pick (&rng, 
                         mbt->g_min_add_funs_inc, 
                         mbt->g_max_add_funs_inc),
                   pick (&rng, 
                         mbt->g_min_add_uf_inc, 
                         mbt->g_max_add_uf_inc),
                   pick (&rng, 
                         mbt->g_min_add_arrayops_inc, 
                         mbt->g_max_add_arrayops_inc),
                   pick (&rng, 
                         mbt->g_min_add_bitvecops_inc, 
                         mbt->g_max_add_bitvecops_inc),
                   pick (&rng, 
                         mbt->g_min_add_inputs_inc, 
                         mbt->g_max_add_inputs_inc));
      BTORMBT_LOG (1, "inc: pick %d ops (add:rel=%0.1f%%:%0.1f%%)",
                   mbt->max_ops, mbt->p_add / 10,
                   mbt->p_release / 10);
      if (mbt->inc)
        BTORMBT_LOG (1, "number of increments: %d", mbt->inc - 1);
...
main (int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
    {
      ...
      if (!strcmp (argv[i], "-h") || !strcmp (argv[i], "--help"))
        ...
        {
          if (++i == argc)
            btormbt_error ("argument to '-o' missing (try '-h')");
          if (argv[i][0] == '-')
          ...
          if (dir)
            ...
            btormbt_error ("given output directory does not exist");
               ...
               || !strcmp (argv[i], "--quit-after-first"))
        {
        ...
        {
          if (++i == argc)
            btormbt_error ("argument to '-m' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument '%s' to '-m' is not a number (try '-h')", argv[i]);
          g_btormbt->g_max_rounds = atoi (argv[i]);
        }
      else if (!strcmp (argv[i], "-t"))
        {
          if (++i == argc)
            btormbt_error ("argument to '-t' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument '%s' to '-t' is not a number (try '-h')", argv[i]);
      ...
      /* boolector options */
        ...
        {
          if (++i == argc)
            btormbt_error ("argument to '-b' missing (try '-h')");
          btoropt = 0;
          ...
          if (!btoropt)
            btormbt_error ("invalid boolector option '%s'", argv[i]);
          if (++i == argc)
            btormbt_error ("argument to '-b' missing (try '-h')");
          val = (int) strtol (argv[i], &tmp, 10);
          if (tmp[0] != 0)
            btormbt_error ("invalid argument to '-b' (try '-h')");
          btoropt->val = val;
      ...
      /* advanced options */
      ...
      else if (!strcmp (argv[i], "--index-bw"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--index-bw' is not a number (try '-h')");
          g_btormbt->g_min_index_bw = atoi (argv[i]);
          if (g_btormbt->g_min_index_bw < MIN_INDEXWIDTH)
            btormbt_error ("min value of '--index-bw' must not be less "
                           "than %d (try '-h')", MIN_INDEXWIDTH);
          if (++i == argc)
            btormbt_error ("argument to '--index-bw' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--muldiv-bw"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--muldiv-bw' is not a number (try '-h')");
          g_btormbt->g_min_muldiv_bw = atoi (argv[i]);
          if (g_btormbt->g_min_muldiv_bw < MIN_MULDIVWIDTH)
            btormbt_error ("min value of '--muldiv-bw' must not be less "
                           "than %d (try '-h')", MIN_MULDIVWIDTH);
          if (++i == argc)
            btormbt_error ("argument to '--muldiv-bw' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--vars-init"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--vars-init' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--vars-inc"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--vars-inc' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--consts-inc"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--consts-inc' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--arrays-inc"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--arrays-inc' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--add-funs"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--add-funs' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--add-arrayops-init"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--add-arrayops-init' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error(
      ...
      else if (!strcmp (argv[i], "--add-arrayops-inc"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--add-arrayops-inc' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--add-bitvecops-init"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--add-bitvecops-init' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--add-bitvecops-inc"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--add-bitvecops-inc' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--add-inputs-init"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--add-inputs-init' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--add-inputs"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--add-inputs' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--ops-init"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--ops-init' is not a number (try '-h')");
        ...
        {
          if (++i == argc)
            btormbt_error ("argument to '--ops' missing (try '-h')");
          if (!isnumstr (argv[i]))
      ...
      else if (!strcmp (argv[i], "--ops-inc"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--ops-inc' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--add-ops"))
        {
          ...
          if (!isnumstr (argv[i]))
            btormbt_error (
                "argument to '--add-ops' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--release-ops-init"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--release-ops-init' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--release-ops-inc"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--release-ops-inc' missing (try '-h')");
          if (!isnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--p-sort-bv"))
        {
          ...
          if (!isfloatnumstr (argv[i]))
            btormbt_error (
                "argument to '--p-sort-bv' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--p-sort-fun"))
        {
          ...
          if (!isfloatnumstr (argv[i]))
            btormbt_error (
                "argument to '--p-sort-fun' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--p-sort-fun-unary"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--p-sort-fun-unary' missing (try '-h')");
          if (!isfloatnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--p-assume"))
        {
          ...
          if (!isfloatnumstr (argv[i]))
            btormbt_error (
                "argument to '--p-assume' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--p-param-arr-exp"))
        {
          if (++i == argc)
            btormbt_error (
                "argument to '--p-param-arr-exp' missing (try '-h')");
          if (!isfloatnumstr (argv[i]))
            btormbt_error (
      ...
      else if (!strcmp (argv[i], "--p-apply-uf"))
        {
          ...
          if (!isfloatnumstr (argv[i]))
            btormbt_error (
                "argument to '--p-apply-uf' is not a number (try '-h')");
      ...
      else if (!strcmp (argv[i], "--output-format"))
        {
          ...
          if (strcmp (argv[i], "btor")
              && strcmp (argv[i], "smt2"))
            btormbt_error (
                "argument to '--output-format' is invalid (try '-h')");
          g_btormbt->output_format = argv[i];
        }
      else if (!isnumstr (argv[i]))
        {
          btormbt_error (
              "invalid command line option '%s' (try '-h')", argv[i]);
        }
      else
        {
  ...
  g_btormbtstats = mmap (0, sizeof (BtorMBTStatistics),
  ...
  mac = hashmac ();
  for (g_btormbt->rounds = 0; 
       g_btormbt->rounds < g_btormbt->g_max_rounds; 
       g_btormbt->rounds++)
    {
      if (!(prev & 1)) prev++;
          ...
          if (g_btormbt->out)
            {
              ...
              sprintf (cmd, "cp %s %s/btormbt-bug-%d.trace",
                       name, g_btormbt->out, g_btormbt->seed);
            }
          else
            {
              cmdlen = 40 + strlen (name) +
                       btor_num_digits_util (g_btormbt->seed);
              BTOR_NEWN (g_btormbt->mm, cmd, cmdlen);
              sprintf (cmd, "cp %s btormbt-bug-%d.trace",
                       name, g_btormbt->seed);
            }

          if (!getenv ("BTORAPITRACE"))
            BTOR_DELETEN (g_btormbt->mm, name, namelen);
      ...
      if ((res == EXIT_ERROR && g_btormbt->quit_after_first)
          || g_btormbt->seeded)
        break;
    }
